...
/**
 * Creates a repeatable CMS item and all of its field records together.
 *
 * Previously the editor created an empty parent first, then created fields
 * one at a time while the user edited. Creating the complete item here gives
 * every card its own parent/child relationship from the start.
 */
export async function createRepeatableItem({ sectionId, sortOrder = 0 }) {
  const session = await requireAdmin()
  ...
  const template = getSectionTemplate(section.sectionKey)
  const fields = template?.defaultItems?.[0]?.fields ?? []
  const parent = await prisma.contentBlock.create({
    data: {
      sectionId,
      fieldKey: 'item',
  ...
  for (const [index, field] of fields.entries()) {
    children.push(await prisma.contentBlock.create({
      data: {
        sectionId,
        parentId: parent.id,
        fieldKey: field.fieldKey,
        blockType: field.blockType,
        // A new item must be independent, not a clone of the template's copy.
        textValue: null,
        sortOrder: index,
        isPublished: true,
        createdById: session.user.id,
  ...
  await writeAudit(session.user.id, 'CREATE_REPEATABLE_ITEM', 'ContentBlock', parent.id, {
    sectionId,
...
export async function upsertBlock(data) {
  ...
  const existing = await prisma.contentBlock.findFirst({
    where: {
      sectionId, fieldKey,
      ...(parentId ? { parentId } : { OR: [{ parentId: null }, { parentId: { isSet: false } }] }),
    },
