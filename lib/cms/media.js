 ...
 *   3. Upload buffer to Cloudinary (uploadFile)
...
export async function uploadMedia(params, fieldRule = null) {
  ...
  // 3. Generate a storage key and upload to Cloudinary
...
export async function deleteMedia(assetId, { force = false } = {}) {
  ...
  if (!asset.storageKey.startsWith('public-images/')) {
    try {
...
export async function listMedia({ page = 1, search = '', mimeType = '' } = {}) {
  ...
  const [assets, total] = await databaseQuery(Promise.all([
