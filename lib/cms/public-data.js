...
import { sectionBlocks } from '@/lib/cms/section-context'
...
const PUBLIC_DATA_TIMEOUT_MS = 12000
let lastDatabaseWarning = 0

function withPublicDataTimeout(operation) {
  return databaseQuery(operation, PUBLIC_DATA_TIMEOUT_MS).catch((error) => {
    // A render reads many sections. Report one safe diagnostic per minute,
    // rather than silently showing defaults or flooding the server logs.
    if (Date.now() - lastDatabaseWarning >= 60000) {
      lastDatabaseWarning = Date.now()
      console.error('[cms-data]', JSON.stringify(authDiagnostic(error)))
    }
    throw error
  })
}
...
// A section heading is metadata for the section itself, never a repeatable
// card. Centralising both rules prevents future templates from rendering a
// heading as an empty/fallback item.
function repeatableParents(blocks = []) {
  return blocks.filter((block) =>
    !block.parentId && blocks.some((child) => child.parentId === block.id)
  )
}

export async function getSectionHeading(slug, sectionKey, fallback = '') {
  try {
    return text(await fetchSection(slug, sectionKey), 'sectionHeading', fallback)
  } catch {
    return fallback
  }
}

// Eyebrow labels belong to the section itself, never to an individual card.
// Keeping them separate prevents a badge from becoming a phantom carousel item.
export async function getSectionBadge(slug, sectionKey, fallback = '') {
  try {
    return text(await fetchSection(slug, sectionKey), 'sectionBadge', fallback)
  } catch {
    return fallback
  }
}
...
async function fetchSection(slug, sectionKey) {
  const scoped = sectionBlocks(sectionKey)
  if (scoped !== null) return resolveContentBlocks(scoped).filter((block) => !block.parentId)
  if (!prisma) return []
  ...
  return resolveContentBlocks(section?.blocks)
}

async function fetchSectionWithChildren(slug, sectionKey) {
  const scoped = sectionBlocks(sectionKey)
  if (scoped !== null) return resolveContentBlocks(scoped)
  if (!prisma) return []
  const section = await withPublicDataTimeout(prisma.section.findFirst({
    where: { page: { slug, isPublished: true }, sectionKey, isVisible: true },
    include: {
...
// Some page designs intentionally let one repeatable section template occur
// more than once. Read every instance when the design renders them as one
// sequence, instead of silently using only the first database match.
async function fetchSectionsWithChildren(slug, sectionKey) {
  const scoped = sectionBlocks(sectionKey)
  if (scoped !== null) return resolveContentBlocks(scoped)
  if (!prisma) return []
  const sections = await withPublicDataTimeout(prisma.section.findMany({
    where: { page: { slug, isPublished: true }, sectionKey, isVisible: true },
    orderBy: { sortOrder: 'asc' },
    include: {
      blocks: {
  ...
  return sections.flatMap((section) => resolveContentBlocks(section.blocks))
}
...
export async function getHomeWhatWeDo() {
  try {
    ...
    if (!allBlocks.length) return HOME_WHAT_WE_DO_FALLBACK
    // sectionHeading is a section-level block, not a card. A valid card is a
    // parent block with child fields (image, title, description and href).
    const parents = repeatableParents(allBlocks)
...
export async function getHomeWhyChc() {
  try {
    ...
    if (!allBlocks.length) return HOME_WHY_CHC_FALLBACK
    // Do not turn sectionHeading into a stack card. It has no child fields,
    // which previously made this renderer display a fallback image.
    const parents = repeatableParents(allBlocks)
    ...
    }).filter((card) => card.heading.trim() || card.paragraph.trim())
  } catch {
...
export async function getHomeHero() {
  try {
    ...
    return {
      ...
      cta1Href:  text(blocks, 'cta1Href', ''),
      cta2Label: text(blocks, 'cta2Label', 'Contact us'),
      cta2Href:  text(blocks, 'cta2Href', ''),
    }
  } catch {
    return {
...
export async function getHomeIntro() {
  try {
    ...
    return {
      ...
      buttonLabel: text(blocks, 'buttonLabel', 'Discuss Tech Requirement'),
      buttonHref:  text(blocks, 'buttonHref', ''),
      linkLabel:   text(blocks, 'linkLabel', 'Explore our services'),
      linkHref:    text(blocks, 'linkHref', ''),
      note:        text(blocks, 'note', 'Reliable insights powered by the latest data.'),
    }
  } catch {
    return {
      ...
      buttonLabel: 'Discuss Tech Requirement', buttonHref: '#contact',
      linkLabel: 'Explore our services', linkHref: '/services',
      note: 'Reliable insights powered by the latest data.',
    }
  }
}
...
export async function getContactData() {
  try {
    ...
    return {
      ...
      officeAddress: text(blocks, 'officeAddress', 'Nanthagopalansamy Nagar, Manthaiyamman Kovil, Cumbum, Tamil Nadu 625516'),
      ...
      mapLat:        text(blocks, 'mapLat',        '9.7255116'),
      mapLng:        text(blocks, 'mapLng',        '77.2723335'),
      mapPopupHtml:  text(blocks, 'mapPopupHtml',  '<div class=infowindow><strong class="mb-3 d-inline-block alt-font">Cloudheard Consultancy Pvt Ltd</strong><p class="alt-font">Nanthagopalansamy Nagar, Manthaiyamman Kovil, Cumbum, Tamil Nadu 625516</p></div>'),
    }
  } catch {
    return {
      officeLabel: 'CHC office',
      officeAddress: 'Nanthagopalansamy Nagar, Manthaiyamman Kovil, Cumbum, Tamil Nadu 625516',
      ...
      mapLat: '9.7255116', mapLng: '77.2723335',
      mapPopupHtml: '<div class=infowindow><strong class="mb-3 d-inline-block alt-font">Cloudheard Consultancy Pvt Ltd</strong><p class="alt-font">Nanthagopalansamy Nagar, Manthaiyamman Kovil, Cumbum, Tamil Nadu 625516</p></div>',
    }
  }
}
...
export async function getOracleDeliveryCapacity() {
  try {
    const blocks = await fetchSection('oracle-hcm', 'serviceCarouselItem')
    return {
...
export async function getDeliveryModelProcessSteps(sectionKey) {
  try {
    const allBlocks = await fetchSectionsWithChildren('our-delivery-model', sectionKey)
    const parents = repeatableParents(allBlocks)
...
export async function getDeliveryModelFaqs() {
  try {
    const allBlocks = await fetchSectionsWithChildren('our-delivery-model', 'faqItem')
    const parents = repeatableParents(allBlocks)
