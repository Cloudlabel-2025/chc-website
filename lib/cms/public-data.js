...
function imageUrl(blocks, fieldKey, fallback = '') {
  return getBlock(blocks, fieldKey)?.mediaAsset?.publicUrl ?? fallback
}
...
const NAV_FALLBACK = [
  { id: '1', label: 'Home',          href: '/',              badge: null, children: [] },
  { id: '2', label: 'Oracle HCM',    href: '/oracle-hcm',    badge: null, children: [] },
  { id: '3', label: 'Applications',  href: '/applications',  badge: null, children: [] },
  { id: '4', label: 'Services',      href: '/services',      badge: 'Hot', children: [] },
  { id: '5', label: 'Give One Hour', href: '/give-one-hour', badge: null, children: [] },
  { id: '6', label: 'About',         href: '/about',         badge: null, children: [] },
  { id: '7', label: 'Contact',       href: '/contact',       badge: null, children: [] },
]
...
export async function getNavData() {
  try {
    ...
    return items.length ? items : NAV_FALLBACK
  } catch {
    return NAV_FALLBACK
  }
}
...
export async function getWhatWeDoNav() {
  try {
    const parent = await prisma.navigationItem.findFirst({
      where: { label: 'What we do', parentId: null },
      include: { children: { where: { isVisible: true }, orderBy: { sortOrder: 'asc' } } },
    })
  ...
  try {
    ...
    return parents.map((parent) => {
      ...
      return {
        ...
        image:       field('image')?.mediaAsset?.publicUrl ?? '/images/oracle-home-hcm.jpg',
...
export async function getHomeWhyChc() {
  try {
    ...
    return parents.map((parent, i) => {
      ...
      return {
        image:     field('image')?.mediaAsset?.publicUrl ?? fb.image,
...
async function getFeatureCards(slug, sectionKey) {
  try {
    ...
    return parents.map((parent) => {
      ...
      const field = (key) => children.find((c) => c.fieldKey === key)
      return {
        img:   field('image')?.mediaAsset?.publicUrl ?? 'https://placehold.co/200x150',
        title: field('title')?.textValue     ?? '',
...
export async function getOracleCapabilities() {
  try {
    ...
    return parents.map((parent) => {
      ...
      const field = (key) => children.find((c) => c.fieldKey === key)
      return {
        img:   field('icon')?.mediaAsset?.publicUrl ?? '',
...
export async function getOracleProductisedServices() {
  try {
    ...
    return parents.map((parent) => {
      ...
      const field = (key) => children.find((c) => c.fieldKey === key)
      return {
        img:   field('image')?.mediaAsset?.publicUrl ?? '',
        title: field('title')?.textValue            ?? '',
...
const OUR_IMPACT_SLIDES_FALLBACK = [
  { title: 'Development',    img: 'https://placehold.co/180x150', desc: 'From startups to enterprises, we craft adaptable web solutions that scale with your business.' },
  { title: 'UX / UI Design', img: 'https://placehold.co/160x150', desc: 'Expertise lies in scalable web solutions for both emerging startups and enterprise.' },
  { title: 'Marketing',      img: 'https://placehold.co/200x150', desc: 'We build real-world web solutions ideal for all project sizes and a range of requirements.' },
  { title: 'Content writing',img: 'https://placehold.co/196x150', desc: 'Deliver web solutions designed to meet the evolving needs of startups alike.' },
  { title: 'Product design', img: 'https://placehold.co/244x150', desc: 'Create scalable web solutions tailored for startups to enterprise-level project needs.' },
  { title: 'Development',    img: 'https://placehold.co/180x150', desc: 'We build flexible web solutions that grow from small startups to large-scale demands.' },
  { title: 'UX / UI Design', img: 'https://placehold.co/160x150', desc: 'From startups to enterprises, we craft adaptable web solutions that scale with your business.' },
  { title: 'Marketing',      img: 'https://placehold.co/200x150', desc: 'We deliver web solutions designed to meet the evolving needs of startups alike.' },
  { title: 'Content writing',img: 'https://placehold.co/196x150', desc: 'We build real-world web solutions ideal for all project sizes and a range of requirements.' },
  { title: 'Product design', img: 'https://placehold.co/244x150', desc: 'From startups to enterprises, we craft adaptable web solutions that scale with your business.' },
]
...
export async function getOurImpactSlides() {
  try {
    ...
    const parents = allBlocks.filter((b) => !b.parentId)
    if (!parents.length) return OUR_IMPACT_SLIDES_FALLBACK
    return parents.map((parent) => {
      const children = allBlocks.filter((b) => b.parentId === parent.id)
      const field = (key) => children.find((c) => c.fieldKey === key)
      return {
    ...
    return OUR_IMPACT_SLIDES_FALLBACK
  }
}
...
export async function getApplicationsStackCards(sectionKey) {
  try {
    ...
    return parents.map((parent, i) => {
      ...
      return {
        img:       field('image')?.mediaAsset?.publicUrl ?? fb.img,
  ...
  try {
    ...
    return parents.map((parent) => {
      ...
      return {
        ...
        photo:         field('photo')?.mediaAsset?.publicUrl ?? 'https://placehold.co/600x756',
      }
    })
  } catch {
