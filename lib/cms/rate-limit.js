...
export async function checkRateLimit(identifier) {
  ...
  return {
    limited: false,
    remaining: MAX_LOGIN_ATTEMPTS - recentFailures,
    resetAt: null,
  }
}
...
/** Clears a successful user's active failure window after a valid sign-in. */
export async function clearLoginFailures(identifier) {
  await prisma.loginAttempt.deleteMany({
    where: { identifier, success: false, attemptedAt: { gte: new Date(Date.now() - WINDOW_MS) } },
  })
}
