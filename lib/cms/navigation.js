...
// MongoDB distinguishes an explicitly stored null from a field that is
// omitted. Older root navigation documents omitted parentId, so both shapes
// must be recognised as top-level items.
export function topLevelNavigationWhere() {
  return { OR: [{ parentId: null }, { parentId: { isSet: false } }] }
}
...
async function getNextSortOrder(parentId) {
  const last = await prisma.navigationItem.findFirst({
    where: parentId ? { parentId } : topLevelNavigationWhere(),
  ...
  for (const item of DEFAULT_NAVIGATION) {
    const parent = await prisma.navigationItem.create({
      data: {
        ...
        parentId: null, isVisible: true, updatedById: userId,
      },
    })
    created.push(parent)
