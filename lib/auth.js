...
const { handlers, auth: nextAuth, signIn, signOut } = NextAuth({
  logger: {
    error(error) {
      console.error('[cms-auth]', JSON.stringify(authDiagnostic(error)))
    },
  },
  providers: [
    Credentials({
  ...
  // AUTH_TRUST_HOST=true behind a trusted proxy, or AUTH_URL set (mirrors
  // Auth.js's own default resolution).
  trustHost:
