...
// Route errors can be generated by Next.js, a proxy, or a storage provider and
// may have an empty/non-JSON body. Never let response parsing crash the CMS UI.
...
export default function MediaLibrary({ onSelect, selectionMode = false }) {
  ...
  async function handleUpload(e) {
    ...
    for (const file of files) {
      ...
      if (!res.ok) {
        const messages = Array.isArray(data.errors) ? data.errors : [data.error ?? 'Upload failed.']
        errs.push(`${file.name}: ${messages.join(', ')}`)
      }
    }
  ...
  async function confirmDelete(asset) {
    ...
    try {
      ...
      const data = await readResponseJson(res)
      if (!res.ok) {
        setError(data.error ?? 'Could not load media details.')
        return
      }
      setDeleteConfirm({ asset, references: data.asset?.referenceCount ?? 0 })
    } catch {
      setError('Could not load media details. Please try again.')
    }
  }
